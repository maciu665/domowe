...
for i in images:
    ...
    #ci = i.crop((0, 0, 260, 1190))
    ci = i
    #cr = old_roughness.crop((0, 0, 260, old_roughness.size[1]))
    cr = old_roughness
    # paste the image into the new image
    new_image.paste(ci, (int((finsize[0] - ci.size[0]) / 2), int((finsize[1] - ci.size[1]) / 2)))
    new_roughness.paste(cr, (int((finsize[0] - cr.size[0]) / 2), int((finsize[1] - cr.size[1]) / 2)))
...
import random   
mosaic = Image.new("RGB", (finsize[0] * 20, finsize[1] * 9), (0,0,0))
roughness_mosaic = Image.new("RGB", (finsize[0] * 20, finsize[1] * 9), (255, 255, 255))
for i in range(9):
    for j in range(20):
...
# rotate both mosaics by 90 degrees
#mosaic = mosaic.rotate(90, expand=True)
#roughness_mosaic = roughness_mosaic.rotate(90, expand=True)

mosaic.save("/home/maciejm/PROJEKT/KONSTRUKCYJNY-200424/TEX/monpelli_olive_mosaic.jpg")
roughness_mosaic.save("/home/maciejm/PROJEKT/KONSTRUKCYJNY-200424/TEX/monpelli_olive_roughness_mosaic.jpg")
